...
export default (ctx, inject) => {
  ...
  if (process.server) {
    ...
    beforeNuxtRender(async ({ Components, nuxtState }) => {
      ...
      //   }
      // })
      let componentsToPrefetch = []
      const r = (component, arr) => {
        if(component.apollo && component.apollo.$init) delete component.apollo.$init
        if(component.apollo) arr.push(component)
        if(component.components) Object.values(component.components).forEach(c => r(c, arr))
      }
      Components.map(c => c.options).forEach(c => r(c, componentsToPrefetch))
      await ApolloSSR.prefetchAll(apolloProvider, componentsToPrefetch, ctx)
